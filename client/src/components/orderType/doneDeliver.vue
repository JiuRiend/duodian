<template>
  <div class="qwer">
    <ul class="uls" v-for="item in donDeliverOrderList" :key="item.id">
      <li>
...
<script>
import { getDonDeliverOrder } from "api/orderType";
...
export default {
  ...
  created() {
    getDonDeliverOrder().then(res => {
      this.donDeliverOrderList = res.result;
    });
  },
  methods: {
    dialogFn(type) {
