<template>
  <div class="qwer">
    <ul class="uls" v-for="item in allOrderList" :key="item.id">
      <li @click="orderTypeFn(item.id)">
        <div class="orderType_top">
          <div>{{ item.order_shop }}</div>
          <div></div>
          <div v-if="item.order_type === 1">已付款</div>
        ...
        <div class="orderType_bom">
          <div></div>
          <div class="orderType_bom_btn" v-if="item.order_type === 4">
            <p>评价</p>
            <p @click="dialogFn(item.order_type)">查看信息</p>
          </div>
          <div class="orderType_bom_btn" v-else-if="item.order_type === 3">
            <p @click="dialogFn(item.order_type)">查看物流</p>
          </div>
          <div class="orderType_bom_btn" v-else>
    ...
    <van-dialog v-model="dialog" title="物流信息">
      <van-steps direction="vertical" :active="0" v-if="showType === 1">
        <van-step>
          <h3>买家已付款</h3>
      ...
      <van-steps direction="vertical" :active="0" v-if="showType === 3">
        <van-step>
          <h3>正在配送...</h3>
          <p>2016-07-10 09:30</p>
        </van-step>
      ...
      <van-steps direction="vertical" :active="0" v-if="showType === 4">
        <van-step>
          <h3>您已确认收货，记得给我们小编好评哦，亲 ❤</h3>
          <p>2016-07-10 09:30</p>
        </van-step>
        <van-step>
          <h3>正在配送...</h3>
...
export default {
  ...
  methods: {
    ...
    orderTypeFn(id) {
      this.$router.push({ path: `/list/orderTypeDetail/${id}` });
    }
  }
};
</script>
